...
Live monitoring and visualization for hackathon presentation
"""
...
class PerformanceDashboard:
    ...
    def calculate_stats(self, window_minutes: int = 5) -> Dict[str, Any]:
        ...
        return {
            ...
            "requests_per_minute": len(recent_metrics) / window_minutes if window_minutes > 0 else 0
        }
    ...
    def render_dashboard(self):
        ...
        if "error" not in stats_1min and "error" not in stats_5min:
            ...
            print(f"   Requests:        {stats_1min['total_requests']:8d}    {stats_5min['total_requests']:8d}")
            print(f"   Success Rate:    {self.format_success_rate(stats_1min['success_rate']):>15} {self.format_success_rate(stats_5min['success_rate']):>15}")
            print(f"   Avg Latency:     {stats_1min['latency']['avg']:8.1f}ms   {stats_5min['latency']['avg']:8.1f}ms")
            print(f"   P95 Latency:     {stats_1min['latency']['p95']:8.1f}ms   {stats_5min['latency']['p95']:8.1f}ms")
            print(f"   Req/Min:         {stats_1min['requests_per_minute']:8.1f}     {stats_5min['requests_per_minute']:8.1f}")
            print()
            ...
            latency_status = "✅" if avg_latency < 20 else "⚠️" if avg_latency < 50 else "❌"
            success_status = "✅" if success_rate >= 0.95 else "⚠️" if success_rate >= 0.85 else "❌"
            ...
            if recent_latencies:
                ...
                print(f"   {min(recent_latencies):4.0f}ms ▕{histogram}▏ {max(recent_latencies):4.0f}ms")
                print()
        ...
        for i, metric in enumerate(reversed(recent_metrics), 1):
            ...
            print(f"   {i:2d}. {timestamp} {status} {latency_str} | "
        ...
        print("   🔗 1,319x faster MCP integration")
        print("   🧠 44 projects analyzed")
    ...
    async def start_monitoring(self, refresh_interval: float = 2.0):
        ...
        try:
            ...
            # Final summary
            if self.metrics:
                final_stats = self.calculate_stats(len(self.metrics) / 10)  # Assume ~10 req/min
...
async def main():
    ...
    print("   Perfect for live demo during hackathon presentation")
    print()
